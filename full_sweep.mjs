// full_sweep.mjs — test all phases every 10° to 1000 yr, find the best
...
console.log('Full phase sweep every 10°, tested to 1000 yr:');
const results = [];
for (let ph = 0; ph < 360; ph += 10) {
  const yr = runToYear(ph, 1000);
  results.push({ ph, yr });
  process.stdout.write(`  ph=${String(ph).padStart(3)}°: ${yr > 1000 ? '1000+ yr ✓' : `ejected yr ${yr}`}\n`);
}

results.sort((a,b) => b.yr - a.yr);
console.log('\nTop 5 best phases:');
for (const r of results.slice(0,5)) {
  console.log(`  ph=${r.ph}°: ${r.yr > 1000 ? '1000+ yr' : r.yr + ' yr'}`);
}
