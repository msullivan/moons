...
import { M_MOON, R_MOON, LUNAR_DIST, AU, M_EARTH, R_EARTH, M_SUN, MOON_PARAMS } from '../bodies.js';
...
// Build analysis objects from MOON_PARAMS (M, R, a, T_d already computed there)
const moons = MOON_PARAMS.map(m => ({
  ...m,
  dir: m.prograde ? 'prograde' : 'retrograde',
  rho:      m.M / (4/3 * Math.PI * m.R**3),
  ang_mean: 2 * m.R / m.a * (180/Math.PI) * 60,
  ang_peri: 2 * m.R / (m.a * (1 - m.e)) * (180/Math.PI) * 60,
  ang_apo:  2 * m.R / (m.a * (1 + m.e)) * (180/Math.PI) * 60,
  g_surf:   G * m.M / m.R**2,
  v_esc:    Math.sqrt(2 * G * m.M / m.R),
  SA_Mkm2:  4 * Math.PI * (m.R / 1e3)**2 / 1e6,
  hill_pct:     m.a / R_HILL * 100,
...
for (const m of moons) {
  ...
  console.log(`  Semi-major:    ${m.a_LD.toFixed(3)} LD  /  period ${m.T_d.toFixed(2)} days`);
