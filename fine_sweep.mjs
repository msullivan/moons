// fine_sweep.mjs — fine-grained phase sweep at a=0.24 LD, 5° steps, to 500 yr
...
function createBodies(phase_deg) {
...
function run(phase, maxYr) {
  const dt=360, YEAR=365.25*86400;
  const chkSteps=Math.round(10*YEAR/dt);
  const bodies=createBodies(phase);
  computeAcc(bodies);
  const qaia=bodies[1], moons=[bodies[2],bodies[3],bodies[4],bodies[5]];
  for (let yr=10;yr<=maxYr;yr+=10) {
    for (let s=0;s<chkSteps;s++) stepVV(bodies,dt);
    for (const m of moons) {
      if (eps(m,qaia)>0) return { ejected:m.name, yr };
    }
  }
  return { ejected:null, yr:maxYr };
}

const MAX_YR = 500;
console.log(`Fine phase sweep, a=0.24 LD, 5° steps, up to ${MAX_YR} yr:`);
console.log('(showing phases that survive > 200 yr)\n');

const results = [];
for (let ph = 0; ph < 360; ph += 5) {
  const r = run(ph, MAX_YR);
  results.push({ ph, ...r });
  if (r.ejected === null || r.yr > 200) {
    const status = r.ejected ? `${r.ejected} ejected yr ${r.yr}` : `✓ all stable to ${MAX_YR} yr`;
    console.log(`  phase=${String(ph).padStart(3)}°: ${status}`);
  }
}

const best = results.filter(r => r.ejected === null || r.yr >= MAX_YR);
console.log(`\nPhases stable to ${MAX_YR} yr: ${best.map(r=>r.ph+'°').join(', ') || 'none'}`);
const topSurvivors = results.sort((a,b) => (b.ejected ? b.yr : MAX_YR) - (a.ejected ? a.yr : MAX_YR)).slice(0,5);
console.log('Top 5 survivors:', topSurvivors.map(r => `${r.ph}°→${r.ejected ? r.yr+'yr' : '500yr+'}`).join(', '));
