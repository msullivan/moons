// outer_long.mjs — long test for promising retrograde outer moon candidates
// Run with: node tests/outer_long.mjs
...
function createBodies(outer_a_ld, M_OUTER) {
  ...
  const bodies = [
    ...
    // Retrograde outer moon: periapsis in +y, retrograde v in +x
    { name:'Outer',   mass:M_OUTER,    x:AU,                 y:OUTER_R_PERI,    z:0, vx:v_outer_peri,    vy:v_earth,  vz:0 },
  ];
...
function run(outer_a_ld, M_OUTER, maxYr) {
  const dt=360, YEAR=365.25*86400;
  const stepsPerCheck=Math.round(10*YEAR/dt);
  const bodies=createBodies(outer_a_ld, M_OUTER);
  computeAcc(bodies);
  const qaia=bodies[1];
  const moons=bodies.slice(2);

  process.stdout.write(`  a=${String(outer_a_ld).padEnd(3)} LD: `);
...
const MAX_YR = 1000;
const SEMIMAJORS = [1.6, 1.7, 1.8, 1.9, 2.0, 2.2, 2.5];
for (const [label, mf] of [['0.10 LM', 0.10], ['0.05 LM', 0.05], ['0.02 LM', 0.02]]) {
  console.log(`\nRetrograde outer moon to ${MAX_YR} yr — mass ${label}, e=0.10`);
  for (const a of SEMIMAJORS) run(a, M_MOON * mf, MAX_YR);
}
