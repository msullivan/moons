// fine_search.mjs
// 1) Fine phase sweep for Inner2 at 0.24 LD (every 15°) tested to 300 yr
// 2) Option B (3-moon system) tested to 1000 yr
...
function createBodies4(inner2_a_ld, inner2_phase_deg) {
  const INNER2_A = inner2_a_ld * LUNAR_DIST;
  ...
  const bodies = [
    ...
    { name:'Inner2',  mass:M_INNER2,
      x:AU+INNER2_R_PERI*Math.cos(phi), y:INNER2_R_PERI*Math.sin(phi), z:0,
      vx:v_inner2_peri*(-Math.sin(phi)), vy:v_earth+v_inner2_peri*Math.cos(phi), vz:0 },
  ];
  return center(bodies);
}

// Option B: 3 moons (Inner1 + Secundus + Primus), no Inner2
function createBodies3() {
  const v_earth = Math.sqrt(G * M_SUN / AU);
  const v_moon_rel = Math.sqrt(G * M_EARTH / LUNAR_DIST);
  const v_inner1_peri = Math.sqrt(G * M_EARTH * (1 + INNER1_E) / INNER1_R_PERI);
  const v_sec_peri    = Math.sqrt(G * M_EARTH * (1 + SECUNDUS_E) / SECUNDUS_R_PERI);
  // Option B uses e=0.20 for Inner1 and Secundus
  const INNER1_E_B = 0.20;
  const INNER1_R_PERI_B = INNER1_A * (1 - INNER1_E_B);
  const SECUNDUS_E_B = 0.20;
  const SECUNDUS_R_PERI_B = SECUNDUS_A * (1 - SECUNDUS_E_B);
  const v_inner1_peri_b = Math.sqrt(G * M_EARTH * (1 + INNER1_E_B) / INNER1_R_PERI_B);
  const v_sec_peri_b    = Math.sqrt(G * M_EARTH * (1 + SECUNDUS_E_B) / SECUNDUS_R_PERI_B);
  const bodies = [
    { name:'Sun',     mass:M_SUN,      x:0,                    y:0,                z:0, vx:0,             vy:0,                                              vz:0 },
    { name:'Qaia',    mass:M_EARTH,    x:AU,                   y:0,                z:0, vx:0,             vy:v_earth,                                        vz:0 },
    { name:'Primus',  mass:M_MOON,     x:AU+LUNAR_DIST,        y:0,                z:0, vx:0,             vy:v_earth+v_moon_rel*Math.cos(PRIMUS_INCLINATION), vz:v_moon_rel*Math.sin(PRIMUS_INCLINATION) },
    { name:'Secundus',mass:M_SECUNDUS, x:AU,                   y:SECUNDUS_R_PERI_B,z:0, vx:-v_sec_peri_b, vy:v_earth,                                        vz:0 },
    { name:'Inner1',  mass:M_INNER1,   x:AU-INNER1_R_PERI_B,  y:0,                z:0, vx:0,             vy:v_earth-v_inner1_peri_b,                         vz:0 },
  ];
  return center(bodies);
}
...
function run(bodies, maxYr, label) {
  ...
  for (let yr=10;yr<=maxYr;yr+=10) {
    ...
    for (const m of moons) {
      if (epsEnergy(m,qaia)>0) {
        if (label) console.log(`  ${label}: ${m.name} ejected yr ${yr}`);
        return yr;
      }
    }
  }
  if (label) console.log(`  ${label}: all stable to ${maxYr} yr ✓`);
  return maxYr + 1;
}

// Fine phase sweep for 4-moon system at 0.24 LD
console.log('=== Fine phase sweep: Inner2 a=0.24 LD, every 15°, to 300 yr ===');
let bestPhase = -1, bestYr = 0;
for (let ph = 0; ph < 360; ph += 15) {
  const bodies = createBodies4(0.24, ph);
  const yr = run(bodies, 300, null);
  const ok = yr > 300;
  console.log(`  ph=${String(ph).padStart(3)}°: ${ok ? 'stable to 300 yr ✓' : `ejected yr ${yr}`}`);
  if (yr > bestYr) { bestYr = yr; bestPhase = ph; }
}
console.log(`\nBest phase: ${bestPhase}° (survived ${Math.min(bestYr,300)}+ yr)`);

// Test best phases to 500 yr
console.log('\n=== Testing all phases that survived 300 yr, to 500 yr ===');
for (let ph = 0; ph < 360; ph += 15) {
  const bodies = createBodies4(0.24, ph);
  const yr300 = run(bodies, 300, null);
  if (yr300 > 300) {
    const bodies2 = createBodies4(0.24, ph);
    run(bodies2, 500, `ph=${ph}°`);
  }
}

// Option B: 3-moon system
console.log('\n=== Option B (3 moons: Inner1 e=0.20, Secundus e=0.20, Primus) to 1000 yr ===');
const bodies3 = createBodies3();
run(bodies3, 1000, 'Option B');
