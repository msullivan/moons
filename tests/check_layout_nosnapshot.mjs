...
// Block snapshot so simulation starts from t=0
await page.route('**/state_200yr.json', route => route.abort());

await page.goto(`http://localhost:${port}/`);
await page.waitForFunction(() => typeof sim !== 'undefined');
await page.waitForTimeout(200);

// Switch to following Sun, zoom out to see solar system
await page.evaluate(() => {
  renderer.followIndex = 0;
  renderer.panX = 0;
  renderer.panY = 0;
  renderer.scale = 1.8e9;
  renderer.render();
});
...
console.log('sim.time:', info.time, 'seconds =', (info.time/86400/365.25).toFixed(4), 'years');
...
await page.screenshot({ path: '/tmp/layout_nosnapshot.png' });
