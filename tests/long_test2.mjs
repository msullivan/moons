// long_test2.mjs — test the winner to 1000 yr, and also sweep 0.22 LD phases
...
function createBodies(inner2_a_ld, inner2_phase_deg) {
  ...
  const bodies = [
    ...
    { name:'Inner2',  mass:M_INNER2,
      x:AU+INNER2_R_PERI*px, y:INNER2_R_PERI*py, z:0,
      vx:v_inner2_peri*tx, vy:v_earth+v_inner2_peri*ty, vz:0 },
  ];
...
function run(inner2_a, phase, maxYr, label) {
  const dt=360, YEAR=365.25*86400;
  const chkSteps=Math.round(10*YEAR/dt);
  const bodies=createBodies(inner2_a, phase);
  computeAcc(bodies);
  const qaia=bodies[1];
  ...
  for (let yr=10;yr<=maxYr;yr+=10) {
    ...
    for (const m of moons) {
      if (eps(m,qaia)>0) {
        console.log(`  ${label}: ${m.name} ejected yr ${yr}`);
        return false;
      }
    }
    if (yr % 100 === 0) {
      const moonStr = moons.map(m => {
        const dx=m.x-qaia.x, dy=m.y-qaia.y, dz=m.z-qaia.z;
        return `${m.name}:${(Math.sqrt(dx*dx+dy*dy+dz*dz)/LUNAR_DIST).toFixed(2)}LD`;
      }).join(' ');
      console.log(`  ${label} yr ${yr}: ${moonStr}`);
    }
  }
  console.log(`  ${label}: all stable to ${maxYr} yr ✓`);
  return true;
}

// Test the winner from first round
console.log('=== Testing a=0.24 LD, phase=315° to 1000 yr ===');
run(0.24, 315, 1000, 'a=0.24 phase=315°');

// Sweep 0.22 LD at various phases
console.log('\n=== Sweep a=0.22 LD to 500 yr ===');
for (let ph = 0; ph < 360; ph += 45) {
  run(0.22, ph, 500, `a=0.22 ph=${ph}°`);
}
