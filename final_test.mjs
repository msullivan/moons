// final_test.mjs — test the 500-yr survivors to 1000 yr
...
const candidates = [25, 70, 115, 205, 220, 305, 315];
const MAX_YR = 1000;
console.log(`Testing 500-yr survivors to ${MAX_YR} yr (a=0.24 LD):\n`);
const outcomes = [];
for (const ph of candidates) {
  process.stdout.write(`  phase=${String(ph).padStart(3)}°: `);
  const r = run(ph, MAX_YR);
  outcomes.push({ ph, ...r });
  if (r.ejected) {
    console.log(`${r.ejected} ejected yr ${r.yr}`);
  } else {
    console.log(`✓ all stable to ${MAX_YR} yr`);
  }
}

const best = outcomes.filter(o => !o.ejected);
if (best.length > 0) {
  console.log(`\nBest phase(s): ${best.map(o=>o.ph+'°').join(', ')}`);
} else {
  const top = outcomes.sort((a,b)=>b.yr-a.yr)[0];
  console.log(`\nNo phase stable to ${MAX_YR} yr. Best: ${top.ph}° → ${top.yr} yr`);
}
