// fine2.mjs — fine sweep around best phases 220° and 70°
...
// Fine sweep around 220°
console.log('Fine sweep around 220° (every 1°), tested to 1000 yr:');
let best220 = { ph: -1, yr: 0 };
for (let ph = 210; ph <= 235; ph++) {
  const yr = runToYear(ph, 1000);
  const s = yr > 1000 ? '1000+ yr ✓✓' : `ejected yr ${yr}`;
  console.log(`  ph=${ph}°: ${s}`);
  if (yr > best220.yr) best220 = { ph, yr };
}

// Fine sweep around 70°
console.log('\nFine sweep around 70° (every 1°), tested to 1000 yr:');
let best70 = { ph: -1, yr: 0 };
for (let ph = 60; ph <= 80; ph++) {
  const yr = runToYear(ph, 1000);
  const s = yr > 1000 ? '1000+ yr ✓✓' : `ejected yr ${yr}`;
  console.log(`  ph=${ph}°: ${s}`);
  if (yr > best70.yr) best70 = { ph, yr };
}

console.log(`\nBest near 220°: ph=${best220.ph}°, ejected yr ${best220.yr}`);
console.log(`Best near 70°:  ph=${best70.ph}°, ejected yr ${best70.yr}`);
