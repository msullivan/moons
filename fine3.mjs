// fine3.mjs — fine sweep around 61° and 77°
...
function runToYear(phase, maxYr) {
  ...
  for (let yr=10;yr<=maxYr;yr+=10) {
    ...
    for (const m of moons) { if (epsBind(m,qaia)>0) return yr; }
  }
  return maxYr + 1;
}

const MAX_YR = 1000;
let globalBest = { ph: -1, yr: 0 };

console.log('Fine sweep around 61° (every 0.5°):');
for (let ph10 = 580; ph10 <= 650; ph10 += 5) {
  const ph = ph10 / 10;
  const yr = runToYear(ph, MAX_YR);
  const s = yr > MAX_YR ? `${MAX_YR}+ yr ✓✓` : `ejected yr ${yr}`;
  console.log(`  ph=${ph}°: ${s}`);
  if (yr > globalBest.yr) globalBest = { ph, yr };
}

console.log('\nFine sweep around 77° (every 0.5°):');
for (let ph10 = 740; ph10 <= 810; ph10 += 5) {
  const ph = ph10 / 10;
  const yr = runToYear(ph, MAX_YR);
  const s = yr > MAX_YR ? `${MAX_YR}+ yr ✓✓` : `ejected yr ${yr}`;
  console.log(`  ph=${ph}°: ${s}`);
...
console.log(`\nGlobal best found: ph=${globalBest.ph}°, survived ${Math.min(globalBest.yr, MAX_YR)}+ yr`);
