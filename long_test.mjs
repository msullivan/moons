// long_test.mjs — test promising phases to 500 yr
...
function run(inner2_a, phase, maxYr) {
...
const candidates = [
  ...
  [0.24, 225],
  [0.24, 315],
  [0.22, 270],  // a=0.22 was stable at 270°
];

const MAX_YR = 500;
console.log(`Testing candidates to ${MAX_YR} yr...\n`);
for (const [a, ph] of candidates) {
  process.stdout.write(`  a=${a} LD, phase=${ph}°: `);
  const r = run(a, ph, MAX_YR);
  if (r.ejected) {
    console.log(`${r.ejected} ejected at yr ${r.yr}`);
  } else {
